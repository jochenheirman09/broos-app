...
/**
 * Finds users who have started a chat but whose `onboardingCompleted` flag is still false,
 * and sets the flag to true.
 */
async function fixStuckOnboarding(): Promise<{ updatedCount: number }> {
  const { adminDb: db } = await getFirebaseAdmin();
  let updatedCount = 0;

  // Find all players whose onboarding is not complete
  const playersQuery = db.collection('users')
    .where('role', '==', 'player')
    .where('onboardingCompleted', '==', false);
  
  const playersSnapshot = await playersQuery.get();

  for (const userDoc of playersSnapshot.docs) {
    // For each player, check if they have any chat history
    const chatsQuery = userDoc.ref.collection('chats').limit(1);
    const chatsSnapshot = await chatsQuery.get();

    // If they have chat history but onboarding is not complete, fix it.
    if (!chatsSnapshot.empty) {
      console.log(`[FIX ONBOARDING] Found user ${userDoc.id} with chats but incomplete onboarding. Updating...`);
      await userDoc.ref.update({ onboardingCompleted: true });
      updatedCount++;
    }
  }
  
  console.log(`[FIX ONBOARDING] Complete. Updated ${updatedCount} users.`);
  return { updatedCount };
}
...
export async function handleFixStuckOnboarding(): Promise<{ success: boolean; message: string; }> {
  ...
  try {
    const { updatedCount } = await fixStuckOnboarding();
    return {
      success: true,
      message: `${updatedCount} gebruikers met een vastgelopen onboarding zijn gerepareerd.`,
    };
  } catch (error: any) {
    console.error("Fix stuck onboarding failed:", error);
    return {
      success: false,
      message: error.message || "An internal server error occurred during repair.",
    };
  }
}
